...
| **去AI味润色** | **按中文小说习惯修订已有正文** | **"去AI味"、"润色第X章"、"一章一章修改"** |
...
**段落节奏**：移动端阅读允许短段，但短段只用于停顿、发现、恐惧、对话转折和结尾钩子；普通动作、环境描写和连续心理过程应适当合并为2-4句一段。详见 `references/writing_principles.md` 的“移动端段落规范”。
...
**功能**：修订已有正文，让文本更接近中文类型小说的自然写法。

**适用场景**：
- 用户要求“去AI味”“润色”“从第X章往前/往后改”
- 用户指出数字、环境描写、callback、词藻等问题
- 已生成章节需要发布前压语言、统一格式

**处理顺序**：
1. 先读目标章节，确认剧情、伏笔和角色信息。
2. 只做贴近原意的修订，不改主线事件、不新增设定。
3. 优先处理硬格式，再处理语言味道，最后检查节奏。
4. 一批修改后执行残留扫描和空白检查。
...
| 类型 | 修订要求 |
|-----|---------|
| 数字格式 | 时间、温度、百分比、数量、楼层、年龄、距离、编号等可量化信息优先用阿拉伯数字：`3点17分`、`39度`、`0.6%`、`17辆` |
| 环境描写 | 环境只保留服务压力、线索、人物处境的锚点；同一场景通常不超过2-3个有效细节 |
| callback | 减少反复回忆同一物件、同一句话、同一画面；保留新信息或推进判断的回收 |
| 比喻和词藻 | 删除生硬比喻、罕用词、抽象漂亮话；优先改成动作、事实、具体观察 |
| 兜底词 | 少用“也许”“仿佛”“某种”“某个”“悄悄地”等含混词，除非角色确实无法判断 |
| 结尾收束 | 避免“他明白了/他不知道/仿佛什么都没发生”式总结，改为动作、证据或悬念 |
| 标点格式 | 对话用中文引号；系统提示、标签、编号保持清晰一致 |

**注意**：引子、梦境、系统视角可以保留少量冷感和神秘感，但仍要避免堆叠比喻和中文数字泛滥。
